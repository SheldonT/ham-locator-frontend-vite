...
import { validateCall, validateFreq } from "../ValidateFunctions";
...
function TableRow({ info, click, optionalFields, editField, hoverEffect }) {
  ...
  const [mode, setMode] = useState(info?.mode ?? "SSB");
  const [sentRep, setSentRep] = useState(info?.sigRepSent ?? "59");
  const [recRep, setRecRep] = useState(info?.sigRepRecv ?? "59");
      ...
      <div className={tableRow.buttonContainer}>
        ...
        <Button
          ...
          clickEvent={() => {
            ...
            setMode(info.mode ?? "SSB");
            setSentRep(info.sigRepSent ?? "59");
            setRecRep(info.sigRepRecv ?? "59");
            setName(info.name ?? "");
            setGrid(info.grid ?? "");
